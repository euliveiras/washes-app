...
function createStaticHandler(routes, opts) {
  ...
  async function submit(request, matches, actionMatch, requestContext, isRouteRequest) {
    ...
    if (!actionMatch.route.action && !actionMatch.route.lazy) {
      ...
      result = await callLoaderOrAction("action", request, actionMatch, matches, manifest, mapRouteProperties2, basename, true, isRouteRequest, requestContext);
      if (request.signal.aborted) {
  ...
  async function loadRouteData(request, matches, requestContext, routeMatch, pendingActionError) {
    ...
    let results = await Promise.all([...matchesToLoad.map((match) => callLoaderOrAction("loader", request, match, matches, manifest, mapRouteProperties2, basename, true, isRouteRequest, requestContext))]);
    if (request.signal.aborted) {
      let method = isRouteRequest ? "queryRoute" : "query";
      throw new Error(method + "() call aborted");
    }
...
react-router-dom/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=http://localhost:3001/build/_shared/chunk-6SJDMTBK.js.map
